...
function cacheRaw(module) {
    ...
    var exported = module.exports,
        keys = Object.keys(exported), len = keys.length;
    for (var i = 0; i < len; i++) {
        if (rawExports.hasOwnProperty(keys[i])) {
            warn("'" + keys[i] + "' is already defined and will be overwritten.");
        }
        rawExports[keys[i]] = exported[keys[i]];
    }
};
    ...
    function dependencyResolved(template) {
        ...
        uses.each(function(use) {
            self.body = self.body.replace(use.match(),
                    exports.templates[template.id](use.params()));
        });
        try {
    ...
    visit(target, null, function(root, child, file) {
        if (/.js$/.test(file)) {
            ...
            new re.InvokeRe(src).each(function(invoke) {
                if (!exports.templates[invoke.template()] && opt['validate']) {
...
function validate() {
    var len = templates.length, unresolved = [];
    for (var i = 0; i < len; i++) {
        if (!template[i].state === UNRESOLVED) {
            unresolved.push(template[i]);
        } else {
            template[i].state = ACTIVE;
        }
    }
